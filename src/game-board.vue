<template>
    <section class="container">
        ...
        <div v-if="stageScreen===1" class="player-list">
            <div v-for="(player, idx) in players" @click="showPlayerConfirm(player)" class="player-select box">
                {{player.name}}
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            targetIsDead() {
                ...
                this.killCount++;
                if(this.killCount === this.players.length - 1) {
                    let winningPlayer = this.players.find(this.getWinner);
                    this.winner = winningPlayer.name;
                    this.stageScreen = 4;
